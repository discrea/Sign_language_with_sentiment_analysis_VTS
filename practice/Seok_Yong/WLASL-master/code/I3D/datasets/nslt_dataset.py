    ...
    for i in range(start, start + num):
        try:
            img = cv2.imread(os.path.join(
                image_dir, vid, "image_" + str(i).zfill(5) + '.jpg'))[:, :, [2, 1, 0]]
        except:
...
def load_rgb_frames_from_video(vid_root, vid, start, num, resize=(256, 256)):
    """영상에서 rgb 프레임을 추출해 ndarray로 반환

    Args:
        vid_root (str): 영상 폴더
        vid (str): 해당 영상 id
        start (int): 시작 프레임
        num (int): 리스트에 저장할 최대 프레임 수
        resize (tuple, optional): 전처리 완료한 영상 크기. Defaults to (256, 256).

    Returns:
        numpy.ndarray: 각 프레임별 rgb값이 담긴 ndarray
    """

    # 영상 경로를 통해 VideoCapture 객체 생성
    ...
    # VideoCapture 객체의 현재 프레임을 현재 함수의 인수로 받은 시작 프레임으로 설정
    ...
    for offset in range(min(num, int(total_frames - start))):
        ...
        if success:
            ...
            if w > 256 or h > 256:
                img = cv2.resize(
                    img, (math.ceil(w * (256 / w)), math.ceil(h * (256 / h))))
...
def load_flow_frames(image_dir, vid, start, num):
    frames = []
    for i in range(start, start + num):
        imgx = cv2.imread(os.path.join(image_dir, vid, vid +
                          '-' + str(i).zfill(6) + 'x.jpg'), cv2.IMREAD_GRAYSCALE)
        imgy = cv2.imread(os.path.join(image_dir, vid, vid +
                          '-' + str(i).zfill(6) + 'y.jpg'), cv2.IMREAD_GRAYSCALE)
...
def make_dataset(split_file, split, root, mode, num_classes):
    ...
    for vid in data.keys():
        ...
        if len(vid) == 5:
            ...
            dataset.append(
                (vid, label, src, data[vid]['action'][1], data[vid]['action'][2] - data[vid]['action'][1]))
...
class NSLT(data_utl.Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgs = load_rgb_frames_from_video(
            self.root['word'], vid, start_f, total_frames)
    ...
    def pad(self, imgs, label, total_frames):
        if imgs.shape[0] < total_frames:
            ...
            if num_padding:
                ...
                if prob > 0.5:
                    pad_img = imgs[0]
                    pad = np.tile(np.expand_dims(pad_img, axis=0),
                                  (num_padding, 1, 1, 1))
